...
Event* NotGate::update(uint64_t current_time) {
	...
	for (auto w : m_inputs) {
		...
		//the next block of code logically inverses the input and outputs it
		if (in == '1') {
			state = '0';
			break;
		}
	...
	//creates a new event if the state changed
	if (state != m_current_state) {
		m_current_state = state;
		uint64_t next = current_time + m_delay;
		e = new Event {next, m_output, state};
	...
	//returns new event that was created (or null if no event was made)
	return e;
}
